import { db, casesTable, documentsTable, findingsTable, crossCaseMatchesTable, courtSessionsTable, courtRoundsTable, motionsTable, categoriesTable, usersTable } from "@workspace/db";
import { eq } from "drizzle-orm";
import { logger } from "./logger";

// Helper function to guarantee a valid demo user exists to bind the cases to
async function getOrCreateDemoUser(tx: any): Promise<number> {
  const existing = await tx
    .select({ id: usersTable.id })
    .from(usersTable)
    .where(eq(usersTable.email, "demo@caselight.ai"))
    .limit(1);
  ...
  const [newUser] = await tx
    .insert(usersTable)
    .values({
      email: "demo@caselight.ai",
      name: "Demo Account",
      password: "dev-bypass-password-secure-123"
    })
    .returning();
...
export async function seedDemoCase(): Promise<void> {
  try {
    ...
    await db.transaction(async (tx) => {
    const demoUserId = await getOrCreateDemoUser(tx);
...
const IL_DEMO_TRANSCRIPT = `...`;
const IL_DEMO_FINDINGS = [];
const IL_DEMO_ROUNDS = [];
const IL_DEMO_VERDICT_SUMMARY = `...`;

async function seedIlCourtSimulation(tx: any, caseId: number, docId: number): Promise<void> {
...
const MN_DEMO_TRANSCRIPT = `...`;
const MN_DEMO_FINDINGS = [];
const MN_DEMO_ROUNDS = [];
const MN_DEMO_VERDICT_SUMMARY = `...`;

async function seedMnCourtSimulation(tx: any, caseId: number, docId: number): Promise<void> {
  const [session] = await tx.insert(courtSessionsTable).values({
    caseId,
    simulationMode: "postconviction_974",
    skepticMode: true,
...
const IN_DEMO_TRANSCRIPT = `...`;
const IN_DEMO_FINDINGS = [];
const IN_DEMO_ROUNDS = [];
const IN_DEMO_VERDICT_SUMMARY = `...`;

async function seedInCourtSimulation(tx: any, caseId: number, docId: number): Promise<void> {
  const [session] = await tx.insert(courtSessionsTable).values({
    caseId,
    simulationMode: "postconviction_974",
    skepticMode: true,
...
const IA_DEMO_TRANSCRIPT = `...`;
const IA_DEMO_FINDINGS = [];
const IA_DEMO_ROUNDS = [];
const IA_DEMO_VERDICT_SUMMARY = `...`;

async function seedIaCourtSimulation(tx: any, caseId: number, docId: number): Promise<void> {
  const [session] = await tx.insert(courtSessionsTable).values({
    caseId,
    simulationMode: "postconviction_974",
    skepticMode: true,
